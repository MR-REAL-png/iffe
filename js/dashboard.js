...
function renderCards(rows){
  ...
  const html=Object.entries(grouped).map(([tgl,txs],gi)=>{
    ...
    const cards=txs.map((r,ri)=>{
      ...
      const bankColor=typeof getBankPillColor==='function'?getBankPillColor(r.pembayaran):null;
      ...
      const tags=[r.pembayaran,r.metode].filter(Boolean).map((t,ti)=>{
        ...
        return isBank?`<span class="dtag" style="background:${pillBg};color:${pillText};border-color:${pillBorder}"><span style="display:inline-block;width:7px;height:7px;border-radius:50%;background:${dotColor};margin-right:4px"></span>${t}</span>`:`<span class="dtag">${t}</span>`;
      }).join('');
...
async function loadTabungan(){
  ...
  try{
    ...
    if(!list.length){
      el.innerHTML=`
        <div class="empty-state"><div class="empty-ico"><svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 21v-8.25M15.75 21v-8.25M8.25 21v-8.25M3 9l9-6 9 6m-1.5 12V10.332A48.36 48.36 0 0 0 12 9.75c-2.551 0-5.056.2-7.5.582V21M3 21h18M12 6.75h.008v.008H12V6.75Z"/></svg></div><div class="empty-title">Belum ada tabungan</div><div class="empty-sub">Tambah target tabungan bersama</div></div>
...
function openStrukDetail(id){
  ...
  const html=`
    <div class="struk">
      ...
      <div class="struk-row"><span>Rekening</span><span style="display:flex;align-items:center;gap:5px">${(()=>{const c=typeof getBankPillColor==='function'?getBankPillColor(r.pembayaran):null;if(!c)return'';return`<span style="width:9px;height:9px;border-radius:50%;background:${c};display:inline-block"></span>`;})()}${r.pembayaran||'—'}</span></div>
